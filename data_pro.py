...
def split_cifar10(data_dir,IF=100):
    if not os.path.exists('%s/images' % data_dir):
        os.makedirs('%s/images' % data_dir)
    imb_factor = 1/IF
    train, test = read_cifar10('%s/cifar-10-python' % data_dir)
    train_num_per_cls = get_img_num_per_cls(10, 5000, imb_factor=imb_factor)
    val_test_num_per_cls = get_img_num_per_cls(10, 500, imb_factor=1)
    ...
    for label in train:
        ...
        for filename, image in train_image_list:
            ...
            save_path = '%s/%s' % (data_dir, filename)
...
def split_cifar100(data_dir,IF=100):
    if not os.path.exists('%s/images' % data_dir):
        os.makedirs('%s/images' % data_dir)
    imb_factor = 1 / IF
    train, test = read_cifar100('%s/cifar-100-python' % data_dir)
    train_num_per_cls = get_img_num_per_cls(100, 500, imb_factor=imb_factor)
    val_test_num_per_cls = get_img_num_per_cls(100, 50, imb_factor=1)
...
if __name__ == '__main__':
    ...
    split_cifar10('%s/cifar10' % data_dir,IF=100)
